	...
	@Override public final void run()
	{
		try
		{
			...
			} else if(var2.contains("<::>"))
			{
				...
				{
					LauncherAuthFrame.frame.entry0("Сервер полон " + var3[0] + " из " + var3[1], 3);
				} else
				{
					LauncherAuthFrame.frame.entry0("Сервер онлайн " + var3[0] + " из " + var3[1], 1);
				}
				LauncherAuthFrame.frame.entry1003.entry0(false);
			} else if(var2.trim().equals("OFF"))
			{
				LauncherAuthFrame.frame.entry0("Сервер недоступен", 2);
				LauncherAuthFrame.frame.entry1003.entry0(false);
			} else if(var2.trim().equals("TechWork"))
			{
				LauncherAuthFrame.frame.entry0("Профилактика", 3);
				LauncherAuthFrame.frame.entry1003.entry0(false);
			} else
			{
