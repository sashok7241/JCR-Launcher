...
if($action == "auth")
{
	...
	$hashPass = $crypt();
	if($injLogin != null && $getPass != null && $client != "null")
	{
		if(!((strCmp($injLogin, $getLogin)) == 0)) die("BadData");
		...
		else
		{
			...
			mysql_query("UPDATE $db_table SET $db_colSesId='$add_seskey' WHERE $db_colUser='$injLogin'") or die("Ошибка");
		}
	} else
	{
		die("BadParams");
